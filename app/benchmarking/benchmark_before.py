...
benchmark_before.py
─────────────────────────────────────────────────────────────────────────────
Benchmarks your ORIGINAL pipeline (no optimisations) for RUN_DURATION seconds
and saves results to results_before.json.

Run this first, then run benchmark_after.py, then compare.py.

Usage:
    python3 benchmark_before.py
─────────────────────────────────────────────────────────────────────────────
"""
...
# ══════════════════════════════════════════════
# MEDIAPIPE  — original settings (model_complexity=1)
# ══════════════════════════════════════════════
...
hands = mp_hands.Hands(
    static_image_mode       = False,   # original default
    max_num_hands           = 1,
    min_detection_confidence= 0.5,
    min_tracking_confidence = 0.5,
    # model_complexity not set → defaults to 1
)
...
# ══════════════════════════════════════════════
# CAMERA  — original (no resolution cap)
# ══════════════════════════════════════════════
cap = cv2.VideoCapture(0)
if not cap.isOpened():
    print("Error: could not open camera.")
    exit(1)
# no cap.set() calls — original behaviour
...
print(f"\nBenchmarking ORIGINAL pipeline for {RUN_DURATION}s...\n")

# ══════════════════════════════════════════════
# MAIN LOOP  — no frame skipping, every frame goes through MediaPipe
# ══════════════════════════════════════════════
while True:
    ...
    # Pre-processing — every frame, no skip
    ...
    cv2.imshow('Benchmark ORIGINAL  [Q=quit]', frame)
...
with open('results_before.json', 'w') as f:
...
──────────────────────────────────────────────────
Saved → results_before.json
Run benchmark_after.py next.
""")
